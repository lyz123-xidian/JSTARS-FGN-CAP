...
def augment(img, label):
    ...
    if np.random.random() < 0.40:
        img = random_jitter(img, 0.5, 0.5, 0.6)
    # if np.random.random() < 0.20:
    ...
    if np.random.random() < 0.10:
        img, label = rotate(img, label, 20, -20)
    if np.random.random() < 0.10:
        img, label = shift(img, label, -10, 10)
...
if __name__ == "__main__":
    ...
    label_path = '/home/data1/jojolee/Tianzhibei2020/Tianzhibei2019OriginalDataset/cropped/512/valid/label/1.png'
